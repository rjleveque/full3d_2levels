...
!
  100 continue
   ...
   ! compute velocity to impose on transducer
   if (time .lt. pulse_span) then
      s = ((time-3.d0*t0wall)/t0wall)**2
      s = amplitude*dexp(-s)
      s = s/rho/dsqrt(lambda/rho)
   else if (time .lt. 2.d0*pulse_span) then
      s = 0.d0
   else ! switch to extrapolation BCs
      go to 110
   end if
     ...
     do k = 1,nfil
        ...
        do j = 1,ncol
           ...
           do i=1,nxl
              ! specify u velocity on the transducer
              if (ycell*ycell + zcell*zcell .lt. trans_halfwidth*trans_halfwidth) then ! for circular transducer
                  ...
                  val(5,i,j,k) = val(5,2*nxl+1-i,j,k)
                  val(6,i,j,k) = val(6,2*nxl+1-i,j,k)
                  val(7,i,j,k) = 2.d0*s - val(7,2*nxl+1-i,j,k)
                  val(8,i,j,k) = val(8,2*nxl+1-i,j,k)
                  val(9,i,j,k) = val(9,2*nxl+1-i,j,k)
               ! zero-order extrapolation for the rest of the boundary
               else
                  do m=1,meqn
...
!     # user-specified boundary conditions go here in place of error output
      write(6,*) '*** ERROR *** mthbc(2)=0 and no BCs specified in bc2amr'
      stop
      go to 299
...
!
  600 continue
!     # user-specified boundary conditions go here in place of error output
      write(6,*) '*** ERROR *** mthbc(6)=0 and no BCs specified in bc3amr'
      stop
      go to 699
